...
func (c *Cursor) WriteState(timestamp time.Time, state StateData, config *RateConfig) (writeError error) {
	...
	if c.lastMutation != nil && timestamp.Sub(c.lastMutation.Timestamp) < config.ChangeFrequency {
	...
	if c.lastState == nil || timestamp.Sub(c.lastSnapshot.Timestamp) >= config.KeyframeFrequency {
		// Make a new snapshot
